...
public class Book {
    ...
    public static String capitalize(String word) {
        ...
        if(split.length == 1){
            ...
            newWord = split2[0].toUpperCase();
            for(int i = 1; i <= split2.length-1; i++){
                newWord += split2[i].toLowerCase();
            }
        }
        if(split.length > 1){
            for(int i = 0; i <= split.length-1; i++){
                newWord += split[i].substring(0,1).toUpperCase()+split[i].substring(1).toLowerCase()+ " ";
            }
        }
    ...
    public void setAuthor(String author) {
        ...
        if(!author.isEmpty()){
            ...
            if(split.length == 1) {
                ...
                this.author = capitalize(split[0])+" "+capitalize(split[1]);
            }
        }
    ...
    public void setTitle(String title) {
        if (!title.contains(" ") && isTitleWord(title) == true) {
    ...
    public static int getUnknownAuthors(){
        int getUnknownAuthors = 0;

        return getUnknownAuthors;
    }
